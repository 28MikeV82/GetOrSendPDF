...
### Логика работы метода
Метод отсылает пример pdf отчета на указанный е-мэйл пользователя. Файл, который необходимо прислать пользователю прикреплен к задаче. Предусмотреть возможность легкой замены файла при необходимости. Для отправки письма необходимо использовать тот же метод, что и в **sendPDF**.
