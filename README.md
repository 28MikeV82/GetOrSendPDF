...
- `email.sendPdf.subject` = Полная информация об автомобиле %grz|vin% - тема письма для метода **sendPDF**, может содержать макросы (см. **Раскрытие макросов**)
- `email.sendPdf.body.template` = /email-sendPdf-template.txt - имя файла с шаблоном тела письма для метода **sendPDF**, может содержать макросы (см. **Раскрытие макросов**) 
- `email.exampleSend.subject` = Полная информация об автомобиле Марка модель госномер - тема письма для метода **exampleSend**, может содержать макросы (см. **Раскрытие макросов**)
- `email.exampleSend.body.template` = /email-exampleSend-template.txt - имя файла с шаблоном тела письма для метода **exampleSend**, может содержать макросы (см. **Раскрытие макросов**)
- `email.exampleSend.report.template` = mycarstory.pdf - относительный путь (от директории заданной `report.template.path`) до образца отчета для метода **exampleSend** 
...
* mycarstory.pdf (пример отчета для метода **exampleSend**)
...
## Метод exampleSend
`/tl-vinwin-backend/vinwin/exampleSend`
...
### Проверка входных параметров
Проверка соответствует проверке входных параметров метода **sendPDF**, отличие только в отсутствии параметров **sts**, **grz**, **vin**.


### Логика работы метода
Метод отсылает пример pdf отчета на указанный е-мэйл пользователя. Файл, который необходимо прислать пользователю прикреплен к задаче. Нужно предусмотреть возможность легкой замены файла при необходимости. Для отправки письма на е-мэйл пользователя необходимо использовать следующий метод платформы:
http://hub.msp-tl.ru/msp-sgw-ws/service/api/email/send

- Тема письма:<blockquote>Полная информация об автомобиле Марка модель госномер</blockquote>
(предусмотреть смену темы при необходимости)

- Тело письма:<blockquote>Вы запросили образец отчёта об истории владения и эксплуатации автомобиля через сайт "Автоистория". Вы можете найти его во вложении к письму. mycarstory.ru</blockquote>
(предусмотреть смену тела письма при необходимости)

- Название файла: mycarstory.pdf (сменить название можно переименовав шаблон, лежащий на сервер)

### Формат ответа
Формат соответствует формату ответа метода **sendPDF**.
